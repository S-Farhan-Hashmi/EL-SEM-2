...
export function createOrUpdateStationMarker(station, stationId, markerMap) {
    ...
    if (marker) {
        ...
        // If popup is already open, re-attach listeners to the new DOM immediately
        if (marker.isPopupOpen()) {
            bindPopupButtons(marker);
        }
...
async function refreshPeakHourBubbles() {
    ...
    for (const [stationId, data] of Object.entries(allData)) {
        ...
        // Only show the circle if it's currently a peak hour
        if (!isCurrentlyPeak) {
            continue;
        }
